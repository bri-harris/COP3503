...
void ReadFromCSVFile(string filePath, vector<DynamicClass::Lego>& output) {
	...
	while (getline(csv, fileLine)) {
		...
		DynamicClass::Lego lego(number, theme, name, minifigs, pieces, usPrice);
		output.push_back(lego);
	}
}

int main() {
	vector<DynamicClass::Lego> vData;
